...
class Vec3f {
   public:
    ...
    [[nodiscard]] constexpr Vec3f normalize() const {
        return scale(1 / length());
    }

    [[nodiscard]] constexpr Vec3f scale(float factor) const {
        return Vec3f(factor * mx, factor * my, factor * mz);
    }

    [[nodiscard]] constexpr Vec3f cap(float max) const {
        float nx = mx;
        float ny = my;
        float nz = mz;
        if (mx > max) nx = max;
...
inline std::ostream& operator<<(std::ostream& os, const Vec3f& a) {
    os << "(" << a.x() << " " << a.y() << " " << a.z() << ")";
    return os;
}
